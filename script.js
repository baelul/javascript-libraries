// fade in code using JQuery
jQuery(window).scroll(function() {
    jQuery('.fade-in').each(function() {
        var toe = jQuery(this).offset().top;
        var boe = jQuery(this).offset().top + jQuery(this).outerHeight();
        var bos = jQuery(window).scrollTop() + jQuery(window).innerHeight();
        var tos = jQuery(window).scrollTop();

        if ((bos > toe) && (tos < boe) && !jQuery(this).hasClass('is-visible'))
        {
...
function createElementProper(incomingJSON, content) {
  ...
  newContentElement.classList.add("fade-in");
